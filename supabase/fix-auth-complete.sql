-- ============================================================
-- ScanPlay — FIX COMPLET Auth Email + Tables + Profils
-- Projet : https://makskleablwrmzhtbejc.supabase.co
--
-- Dashboard → SQL Editor → New query → Coller TOUT → Run
--
-- APRÈS ce SQL, configure aussi (Dashboard, pas SQL) :
...
-- ── 4. FONCTION : créer profil à la connexion ───────────────
...
-- ── 6. BACKFILL : comptes existants sans profil ─────────────
...
-- ── 7. FIX EMAIL : confirmer les comptes en attente ─────────
-- Si "Confirm email" est activé dans Supabase, la connexion échoue sans confirmation.
...
select 'scanplay_profiles' as table_name, count(*) as rows from public.scanplay_profiles
union all
select 'scanplay_decks', count(*) from public.scanplay_decks
union all
select 'scanplay_mistakes', count(*) from public.scanplay_mistakes
union all
select 'scanplay_user_stats', count(*) from public.scanplay_user_stats;

 ...
 * Fixes classic DOCUMENT_TEXT_DETECTION glitches on EN infinitives + row mis-pairs.
 */
...
/**
 * "To know savoir" / "To tell dire, raconter" → EN head + FR tail.
 * Returns null if the cell is already a clean infinitive or not this pattern.
 */
export function splitFusedEnFrRow(text: string): { term: string; definition: string } | null {
  ...
  /* Need at least some FR signal so "To look forward" isn't split. */
  if (!FR_LEX.test(definition) && definition.split(/\s+/).length > 2) return null;
  return { term, definition };
}

/** Dedupe key: "To seem sembler" and "To seem" collapse to the same head. */
...
/**
 * Bottom-of-sheet failure mode: consecutive full rows paired as term→definition
 * ("To seem sembler" → "To feel se sentir"). Expand into real EN→FR cards.
 */
...
/** True when a Vision/OCR pair looks like a full-row mash, not a clean EN→FR card. */
...
export function sanitizeVocabExtractPairs<T extends VocabPairLike>(pairs: T[]): T[] {
  ...
  for (const p of expanded) {
    ...
    /* Definition should not be another English infinitive row. */
